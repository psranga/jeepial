    ...
    if ok == true then
      for j, dep in ipairs(p.deps) do
        if not built_values[dep] then
          ok = false
          dlog4(me, ' linenum=', i, ' dep=', dep, ' is not ready. code=', p.code)
          break
        end
      end
      if ok == true then
        dlog4(me, ' linenum=', i, ' dsts=', p.dsts, ' all deps ready!')
      end
    end
...
function num_ready_lines_with_operation(ready_lines, g, operation)
  ...
  dlog4('num_ready_lines_with_operation', ' n=', n, ' operation=', operation, ' #ready_lines=', #ready_lines)
  return n
end
...
function execute_line(g, linenum, dst_values)
  ...
  local lut = {
    ...
    {op_patt='update', dsts_patt='level2', fn=function() return 1 end},
  ...
  for i, t in ipairs(lut) do
    ...
    if match then
      dlog2(me, '  executing lut entry: ', t)
      local r = t.fn(line)
...
function run_program(g)
  ...
  while true do
    ...
    dlog2(me, 'ready_lines=', ready_lines)
    for i = 1, #ready_lines do
    ...
    if num_ready_lines_with_operation(ready_lines, g, 'precondition') == #ready_lines then
      dlog('run_program', 'all ready lines are preconditions. Done with this epoch.')
      dlog('run_program', 'value updates for next epoch:\n  #n=', #value_updates, '\n', join(map(value_updates, function(x, i) return '  ' .. i .. ' ' .. dlog_snippet(x) end), '\n'))
      break
    end
    ...
    while #ready_lines > 0 do
      ...
      if new_values then
        dlog(me, 'executed line ', linenum, ': new_values=', new_values, ' op=', line.operation)
        ...
        dlog(me, 'executed line ', linenum, ': NO new values. op=', line.operation)
      end
    end
