...
function find_ready_lines(g, built_values, excluded_operations)
    ...
    if ok == true then
      for j, dep in ipairs(p.deps) do
        if not built_values[dep] then
          ok = false
          dlog('find_ready_lines', ' linenum=', i, ' dep=', dep, ' is not ready. code=', p.code)
          break
        end
      end
      if ok == true then
        dlog('find_ready_lines', ' linenum=', i, ' dsts=', p.dsts, ' all deps ready!')
      end
    end
  ...
  dlog(me, 'ready_lines=', r)
  for i = 1, #r do
    dlog(me, '  linenum=', r[i], ' ', g.lines[r[i]].code)
  end
  return r
end
...
function execute_line(g, linenum, dst_values)
  ...
  local lut = {
    ...
    {op_patt='build', dsts_patt='xml', code_patt='wget', fn=function() v_level = v_level + 0; return 'xmltext' end},
    ...
    {op_patt='pbuild', dsts_patt='url', code_patt='unpack.*indexes', fn=function() return {'indexl1.1', 'index1.2', 'index1.3'} end},
  ...
  for i, t in ipairs(lut) do
    dlog('execute_line', '  considering: ', t)
    local match = true
...
function parsed_line_to_source(p)
  return p.linenum .. ' ' .. p.operation .. ' ' .. join(p.dsts, ', ') .. ': ' .. p.code .. '  -- ' .. join(p.deps, ', ')
end
...
function run_program(g)
  ...
  local debug_info = join(map(g.lines, parsed_line_to_source), '\n')
  dlog('run_program', 'got:\n', debug_info, '\n')
  while true do
    ...
    while #ready_lines > 0 do
      ...
      dlog(me, 'executing line ', linenum, ': ', line)
      local new_values = execute_line(g, linenum, built_values)

      if new_values then
        dlog(me, 'got new values: op=', line.operation, ' ', new_values)
